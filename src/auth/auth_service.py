...
async def get_roles(session: AsyncSession, skip: int = 0, limit: int = 100) -> List[auth_models.Role]:
    query = select(auth_models.Role).offset(skip).limit(limit)
    result = await session.execute(query)
    return result.scalars().all()

async def create_user(session: AsyncSession, user: auth_schemas.UserCreate) -> auth_models.User:
...
async def get_users(session: AsyncSession, skip: int = 0, limit: int = 100) -> List[auth_models.User]:
